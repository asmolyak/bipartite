...
def node_list_to_check(G,holdings,prices,min_value,liabilities,failure_in = 'bank',node_to_check=[]):
    if len(node_to_check) == 0:
        node_to_check = set()
    if failure_in == 'bank':
    ...
    return G,node_to_check,holdings

def eval_network(G,holdings,prices,min_value,liabilities,impact,failure_in='bank'):
    inner_loop=0
    G,nodes_to_check,holdings = node_list_to_check(G,holdings,prices,min_value,liabilities,failure_in,node_to_check=[])
            ...
            for neighbors in G.neighbors_iter(node):
                if G.node[neighbors]['kind']=='bank':
                    ...
                    if G.get_edge_data(node,neighbors)['kind'] == 'competitor':
                        G.node[neighbors]['price']/=((1-impact)/(G.get_edge_data(node,neighbors)['strength']))
                        prices[int(neighbors[1:])]/=((1-impact)/(G.get_edge_data(node,neighbors)['strength']))
                    elif G.get_edge_data(node,neighbors)['kind'] == 'partner':
                        G.node[neighbors]['price']*=(0.8*(1-impact)/(G.get_edge_data(node,neighbors)['strength']))
                        prices[int(neighbors[1:])]*=(0.8*(1-impact)/(G.get_edge_data(node,neighbors)['strength']))
#        nnodes = np.where(prices*sum(holdings)-min_value <0)[0]
#        if sum (prices*sum(holdings)-min_value < 0) and any([G.node['a'+str(n)]['value']>0 for n in nnodes ]):
        G,nodes_to_check,holdings = node_list_to_check(G,holdings,prices,min_value,liabilities,failure_in,node_to_check=nodes_to_check)
    print inner_loop
...
a = init_network(banks=banks,assets=assets,holdings=holdings,internals=True,num_of_internals=2000)
...
for imp in np.arange(0.001,0.015,0.0005):
    a = copy_a.copy()
    holdings = hold_cop.copy()
    prices = copy_prices.copy()
    a, prices = hit_network(G=a,prices=prices,prob_to_hit=0.2,hit=0.78)
    hold_cop = holdings.copy()
    ...
    print sum(np.sum(hold_cop,0)>0),sum(np.sum(holdings,0)>0)
    container[imp] = (num_of_iters,sum(sum(holdings>0)))
print datetime.now() - start
pd.DataFrame(container).T[0].plot()
pd.DataFrame(container).T[1].plot(secondary_y=True,style='.')