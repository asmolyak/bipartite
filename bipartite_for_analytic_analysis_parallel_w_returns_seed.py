...
#    gather_iters_by_avdeg = fig2.gca()
    ...
    num_of_assets = 10000
    if fract_threshold:
        health_threshold = np.arange(0.6,1.0001,0.0005)
    else:
        health_threshold = np.arange(0,7.001,.005)#, th_base/((1+th_base)**fract_threshold)#np.arange(0,1,0.01)#
#        hit_range = [0.001]#np.arange(0,.01,0.0001)
    av_deg_range = np.arange(4,14,0.5)#np.logspace(np.log10(0.01),np.log10(18),100)
    power = 2.5
    min_holdings = 5
...
if __name__ == "__main__":
    ...
    pool.close()
    pool.join()
    with gzip.GzipFile('results'+str(date.today())+'.pgz', 'w') as h:
